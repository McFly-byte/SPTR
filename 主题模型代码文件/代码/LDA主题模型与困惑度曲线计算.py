...
def duqu1():
    ...
    cur = conn.cursor()
    try:
        #cur.execute("SELECT * FROM `专利31000` WHERE `时间` >= 2000 AND `时间` <= 2021")
        cur.execute("SELECT * FROM `专利31000` WHERE `时间` >= 2020 AND `时间` <= 2021 ")  #选定时间窗
...
def englishfenci(mytext):
    ...
    cizu = []
    biaozhun = []
    f = open("D:\desktop\科研训练\主题分析/6.txt", "r")#这个文件是自己子自定义添加的分词典
...
pyLDAvis.save_html(pic, '2008_lda_pass' + str(n_topics) + '.html')'''
