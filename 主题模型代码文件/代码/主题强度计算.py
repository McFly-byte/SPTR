...
import re
def duqu1(year):
    conn = pymysql.connect(host='localhost', port=3306, user='root', password='666152', db='patent')
    cur = conn.cursor()
    try:
...
def duqu():
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT * FROM `专利31000` WHERE `时间` >= 2000 AND `时间` <= 2021 ORDER BY `时间`  ")  # 多少条记录
        data = cur.fetchall()
...
def englishfenci(mytext):
    ...
    cizu = []
    biaozhun = []
    f = open("D:\desktop\科研训练\主题分析/6.txt", "r")
...
def plot(zqd):
    ...
    plt.savefig('D:\desktop\图\人脸识别\强度图/曲折.png')
    ...
    plt.xlabel('year')
    plt.ylabel('Intensity of theme')
    plt.savefig('D:\desktop\图\人脸识别\强度图/下降.png')
    ...
    plt.xlabel('year')
    plt.ylabel('Intensity of theme')
    plt.savefig('D:\desktop\图\人脸识别\强度图/上升.png')
